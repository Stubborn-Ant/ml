...
def kMeans(dataSet, k, distMeas=distEclud, createCent=randCent):
    m = shape(dataSet)[0]
    clusterAssment = mat(zeros((m, 2)))  # create mat to assign data points 簇分配结果矩阵,一列记录簇索引值,一列记录存储误差
    ...
    while clusterChanged:
        ...
        for i in range(m):  # for each data point assign it to the closest centroid
            ...
            if clusterAssment[i, 0] != minIndex:
                clusterChanged = True  # clusterAssment任意一项有变化就认为簇有变化 即下边几行代码更新簇中心后clusterAssment的任意一行都没变化则终止
            clusterAssment[i, :] = minIndex, minDist ** 2
        # print 'before change centroids:', centroids
...
if __name__ == '__main__':
    # 测试kmeans
    # dataMat = mat(loadDataSet('myTestSet.txt'))
    ...
    # 测试biKmeans
    # dataMat = mat(loadDataSet('myTestSet2.txt'))
